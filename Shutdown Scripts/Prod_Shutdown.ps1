...
#Get all the production VM's
Write-Host "**** List of all the production VM's *******************"
Get-VM -Tag Production
$ProdVM = Get-VM -Tag Production



#ShutDown the Production VM's
foreach ( $PoweredOnVm in $ProdVM) {
...
#Check whether the Prod VM's are still on, if they are then shut them down
foreach ( $PoweredOnVm in $ProdVM) {
    $vminfo = get-view -Id $PoweredOnVm.ID
...
$powerstate | Export-Csv -Path C:\temp\Prodpowerstate_report_$CurrentDate.csv -NoTypeInformation -UseCulture

$file = "C:\temp\Prodpowerstate_report_$CurrentDate.csv"
...
$msg.Subject = "Notification for the Production Servers Shutdown"
$msg.Body = "Attached is the Production shutdown report "
