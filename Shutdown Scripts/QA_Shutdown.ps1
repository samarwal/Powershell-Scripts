...
#Get all the Quality Assurance VM's
Write-Host ""
Write-Host "**** List of all the Quality Assurance VM's *******************"
Get-VM -Tag QualityAssurance
$QAvm = Get-VM -Tag QualityAssurance



#ShutDown the Quality Assurance VM's
foreach ( $PoweredOnVm in $QAvm) {
...
#Check whether the QA VM's are still on, if they are then shut them down
foreach ( $PoweredOnVm in $QAvm) {
    $vminfo = get-view -Id $PoweredOnVm.ID
...
$powerstate | Export-Csv -Path C:\temp\QApowerstate_report_$CurrentDate.csv -NoTypeInformation -UseCulture

$file = "C:\temp\QApowerstate_report_$CurrentDate.csv"
...
$msg.Subject = "Notification for the Quality Assurance Servers Shutdown"
$msg.Body = "Attached is the Quality Assurance shutdown report "
